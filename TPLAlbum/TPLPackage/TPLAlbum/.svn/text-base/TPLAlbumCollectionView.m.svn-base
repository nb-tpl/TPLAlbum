...
@synthesize assetsGroup = _assetsGroup,choosePhotosArray = _choosePhotosArray;
@synthesize maxChooseNumber = _maxChooseNumber,chooseCount = _chooseCount;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _photosCollectionView.autoresizingMask =UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        //是否超过最大多选数
        if (_maxChooseNumber > _chooseCount)
        {
        ...
        {
            UIAlertView * alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"warmSign", nil) message:[NSString stringWithFormat:@"最多选%d张照片",_maxChooseNumber] delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil];
            [alert show];
        }
    }
    else
    ...
    //点击Block
   __weak TPLAlbumCollectionView *  tplAlbumCollectionView = self;
    if (_cellClickedBlock != nil)
    {
        self.cellClickedBlock(tplAlbumCollectionView,indexPath);
    }
}
